...
package body TTC is
   ...
   begin
      loop
         ...
         Storage.Get (S); -- se podría recoger el reloj después del get (recomendación Juan Rafael)
         Send (S); -- send data
         STM32.Board.Orange_LED.Toggle; -- toggles orange led
         Next_Time := Next_Time + Period; -- calculate time for next activation
      end loop;
   end TM_Task;
